            ...
            if (DOB.active() && cntr !== 1781) {
                ...
                if (checkLine(HAS_ELSE_IF, line)) {
                    ...
                    if (nextDeletableLine['start']['line'] === cntr) {
                        ...
                        // let updateLine = correctIndention && deleteIfExpression !== '' && cntr < 5003 ? deleteIfExpression.replace("if (", "if (!(" ).replace(") {", ")) {")  : line; // ex ''
                        let newLine = line;
                        if (rewriteElse) {
                        ...
                        // }
                        LOB.update(newLine, `#98 END ${DOB.first()}`, STATUS_REMOVED);
                    }
                    ...
                    if (checkLine(HAS_IF, line)) {
                        if(forceDelete === true){
                            ...
                            LOB.update('', "#101a", STATUS_REMOVED);
                        } else {
                            LOB.update(line, "#101b", STATUS_POTENTIAL);
                        }
